...
CREATE PROCEDURE dbo.GenerateSnowflakeDdlDynView
	(
		@SourceDatabaseName VARCHAR(255) = 'WcsOds_SnowFlake'
...
DECLARE @SourceDatabaseName VARCHAR(255) = 'WcsOds_SnowFlake'
		,@OutputPath VARCHAR(MAX) = '\\qa14nas\CSG-Analytics\OdsFileExtracts\SmartAdvisor\Snowflake_Replication\Snowflake_Admin\View'
		,@ProcessId INT = 0
		,@Debug BIT = 1 
*/ 
 
SET NOCOUNT ON;
	-- @ProcessId INT = 0 --'$(processid_)'
...
END TRY
BEGIN CATCH
PRINT 'View Could Not be built...Make sure table exists and no Views have been created on it.'
END CATCH
