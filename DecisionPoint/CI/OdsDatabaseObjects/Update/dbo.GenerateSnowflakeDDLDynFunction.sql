...
CREATE PROCEDURE dbo.GenerateSnowflakeDDLDynFunction
	(
		 @SourceDatabaseName VARCHAR(255) = 'AcsOds_SnowFlake'
...
SET @Query =' SELECT @srcColumnList =  COALESCE(@srcColumnList+CHAR(13)+CHAR(10)+CHAR(9)+ ''	,'','''')+
			CASE 
				WHEN COLUMN_NAME LIKE ''%[^a-Z0-9_]%'' THEN ''t.[''+COLUMN_NAME+'']''
				ELSE ''t.''+COLUMN_NAME END  
...
SELECT DISTINCT I.COLUMN_NAME,I.ORDINAL_POSITION 
